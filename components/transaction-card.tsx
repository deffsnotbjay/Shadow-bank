...
import type { Transaction } from "@/hooks/use-shadow-storage"
...
export function TransactionCard({ transaction, onPost, onReverse, isAdmin = false }: TransactionCardProps) {
  const statusConfig = {
    ...
    >
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm text-blue-600 font-medium">{transaction.accountId}</p>
            <p className="text-xs text-blue-500">{new Date(transaction.timestamp).toLocaleDateString()}</p>
          </div>
        </div>
      ...
      <div className="mb-4">
        <p className="text-3xl font-bold text-blue-900">₦{transaction.amount.toLocaleString()}</p>
      </div>

      {isAdmin && transaction.status !== "reversed" && (
